...
- Además, los tipos de datos más habituales en Ciencias Sociales on los **datos tabulares**.
...
- Es el paquete [`rio`](https://github.com/leeper/rio). En su web pone que el paquete **rio** es "A Swiss-Army Knife for Data I/O".
...
- Te has rendido, bueno ...., primero tienes que instalar el paquete, luego cargarlo en meoria y ya puedes lamar al fichero de datos `penguins`
...
- Tenéis que usar la función `rio::export()`. Esta función tambien tiene varios argumentos, pero los princiaples son los primeros, llamados `"x"` y `"file"`
...
- Otra forma de hacerlo. Tiene ventajas, y es la forma que usaremos cuando nos aconstumbremos
...
Esta vez ya no hay solución!!! Tienes que aconstumbrarte a escribir en R. 
...
Es importante que diferencieis entre descargarse un fichero y cargar el fichero en R, cargar el fichero en memoria de R
...
Podemos descargar y luego cargar en memoria
...
```{r, eval = FALSE}
iris_1 <- import(here::here("pruebas", "my_iris_pinguinos.xlsx") #- solo importa el primer libro
                 
pinguinos_1 <- import(here::here("pruebas", "my_iris_pinguinos.xlsx", 
                      `sheet = 2`) 
                      
                      
pinguinos_2 <- import(here::here("pruebas", "my_iris_pinguinos.xlsx", 
                      `sheet = "pinguinos"`) 
```
...
- ... pero al grabar en estos formatos guardas los datos, PERO no guardas los metadatos; por ejmplo, si una columna la has definido como un factor o como integer, esta información se perderá. En estos casos, una solución es usar el formato propio de R.
...
- Para cargar en memoria un fichero `.rda` o `.RData`  tienes que usar la función `load()`
...
Vamos a arreglar un poco la tablas
