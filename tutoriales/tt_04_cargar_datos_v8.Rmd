...
Para importar/exportar datos vamos a usar funciones de varios packages, así que tenemos que saber como acceder a su documentación, pero esto ya se vió en el tutorial sobre R-base.
...
El paquete `rio` es muy fácil de usar, pero en realidad lo que hace es llamar/utilizar otros paquetes para hacer el trabajo. Por ejmeplo para exportar/importar a SPSS usa el paquete `heaven`, para Matlab usa el paquete `rmatio`, así que si quieres imporatr datos de Matlab tendrás que tenerlo instalado. Al cargar el pauqte rio con `library(rio` nos avisará de si nos falta algun paquete y si queremos los instalara por nosotros con la función `rio::install_formats()`
...
En [este post](http://ritsokiguess.site/docs/2018/03/25/today-on-twitter-i-learned/) también explican que `rio` puede ser una alternativa a `rvest` y/o `xml2` para impotar tablas html en internet. Por ejemplo:
...
A veces, la función de R-base `download.file` puede tener problemas si el protocolo es https. En estos casos, la función `dowload()` del pkg dowloader puede solucionarlo:
...
aa <- pjpv2020.01::pjp_f_valores_unicos(df)     #- ver los valores unicos de cada columna
```
